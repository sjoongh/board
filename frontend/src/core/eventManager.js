export const EventManager = (() => {
  ...
  const addEventHandler = (className, event, handler) => {
      ...
      if (eventMapper[event].some((v) => v.className === className))
          eventMapper[event] = eventMapper[event].filter((v) => v.className !== className);
  ...
  const regist = () => {
      ...
      Object.keys(eventMapper).forEach((event) => {
          const eventHandler = (e) => {
              ...
              const targets = candidates.filter((candidate) => e.target.classList.contains(candidate.className) ||
